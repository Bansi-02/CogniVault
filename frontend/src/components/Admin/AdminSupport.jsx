...
const AdminSupport = () => {
  ...
  const fetchTickets = async () => {
    try {
      const response = await axios.get(`${PYTHON_URL}/api/support`);
  ...
  const handleReply = async (e) => {
    ...
    setReplying(true);
    try {
      await axios.post(`${PYTHON_URL}/api/support/${selectedTicket._id}/reply`, {
    ...
    <div className="max-w-6xl mx-auto h-full flex flex-col">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-400">Manage and respond to enterprise user inquiries.</p>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 bg-[#0b101d] border border-gray-800 rounded-2xl flex flex-col h-[700px]">
            {selectedTicket ? (
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  {selectedTicket.status === 'Closed' ? (
                    ...
                    <form onSubmit={handleReply} className="flex flex-col h-full">
                      <label className="block font-bold text-white mb-2">Compose Reply</label>
                      <textarea
                        required
                        value={replyText}
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
                ...
                <p>Select a ticket from the inbox to view details</p>
              </div>
            )}
          </div>
      </div>
